...
public static class LayoutUtility {
    ...
    public static void LoadAmantLayout() {
        string path = "Assets/Editor/Layouts/AmantLayout.wlt";
        path = Path.Combine(Directory.GetCurrentDirectory(), path);
        GetMethod(MethodType.Load).Invoke(null, new object[] { path, false });
    }
    
    public static void LoadTilePlacerLayout()
    {
        string path = "Assets/Editor/Layouts/AmantTilePlacerLayout.wlt";
        path = Path.Combine(Directory.GetCurrentDirectory(), path);
        GetMethod(MethodType.Load).Invoke(null, new object[] { path, false });
    }
